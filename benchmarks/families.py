...
# ── Rosenbrock (non-parametric stress test) ────────────────────────────
...
def make_rosenbrock_bilinear(dim: int = 5, seed: int = 0) -> BenchmarkProblem:
    ...
    strict convex-concave assumption (Assumption 3.1) of the Minimax-AIPE
    paper.  Included as a challenging stress-test for algorithm robustness.

    Analytical gradient and Hessian are provided to avoid XLA compilation
    hangs that arise when ``jax.hessian(f)`` is traced through the nested
    summation inside the AIPE inner loop.  The Hessian is block-tridiagonal
    (bandwidth 1), computed in O(n) time.
    """
    from minimax_aipe import MinimaxProblem
    ...
    D = 4.0
    _n = dim  # captured as a static Python int for the closures
    ...
    def grad_f(x, y):
        """Analytical gradient of the Rosenbrock + bilinear objective.

        With z = x + 1, the residuals r_i = z_{i+1} − z_i² (i=0..n−2):

          ∂R/∂z_0       = −400 z_0 r_0 − 2(1 − z_0)
          ∂R/∂z_j       = 200 r_{j−1} − 400 z_j r_j − 2(1 − z_j)   (1 ≤ j ≤ n−2)
          ∂R/∂z_{n−1}   = 200 r_{n−2}

        Since z = x + 1, the Jacobian ∂z/∂x = I, so ∂R/∂x = ∂R/∂z.
        """
        z = x + 1.0
        grad_rosen = jnp.zeros(_n)
        if _n > 1:
            r = z[1:] - z[:-1] ** 2  # residuals, length n-1
            # Contribution from 100·r_i²  →  chain through z_i
            grad_rosen = grad_rosen.at[:-1].add(-400.0 * z[:-1] * r)
            # Contribution from 100·r_i²  →  chain through z_{i+1}
            grad_rosen = grad_rosen.at[1:].add(200.0 * r)
            # Contribution from (1 − z_i)²
            grad_rosen = grad_rosen.at[:-1].add(-2.0 * (1.0 - z[:-1]))
        return grad_rosen + A @ y, -(A.T @ x)

    def hessian_f(x, y):
        """Analytical Hessian — block-tridiagonal in the x-block.

        Diagonal entries:
          H[0,0]         = 1200 z_0² − 400 z_1 + 2
          H[j,j]         = 1200 z_j² − 400 z_{j+1} + 202   (1 ≤ j ≤ n−2)
          H[n−1,n−1]     = 200

        Off-diagonal (bandwidth 1):
          H[j, j+1] = H[j+1, j] = −400 z_j               (0 ≤ j ≤ n−2)

        All other entries are zero.  The y-block and cross-blocks are
        constant (zero and A respectively), matching the bilinear coupling.
        """
        z = x + 1.0
        ...
        diag = jnp.zeros(_n)
        if _n > 1:
            diag = diag.at[0].set(1200.0 * z[0] ** 2 - 400.0 * z[1] + 2.0)
            if _n > 2:
                diag = diag.at[1:-1].set(
                    1200.0 * z[1:-1] ** 2 - 400.0 * z[2:] + 202.0
                )
            diag = diag.at[-1].set(200.0)

            # ── Sub/super-diagonal entries ────────────────────────────
            off = -400.0 * z[:-1]  # length n-1
        ...
        zeros_yy = jnp.zeros((_n, _n))
        return ((H_xx, A), (A.T, zeros_yy))
