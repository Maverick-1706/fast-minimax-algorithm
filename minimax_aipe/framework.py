...
from minimax_aipe.len import make_lazy_crn_npe_oracle, len_restart
...
def solve(
    ...
    *,
    ...
    params = _compute_loop_params(problem, epsilon, gamma, npe_T_factor, m_lazy=m_lazy)
...
def _solve_saddle_subproblem(
    ...
    *problem* is the h-subproblem (already has ρ_h = ρ_orig + γ in its
    ...
    # Per Theorem E.1 / E.3: CRN parameter = 2ρ of the subproblem.
    ...
    if M_saddle == "npe":
        ...
        return npe_restart(
            ...
            T=inner_T, gamma=npe_gamma, S=params.S_inner,
            project=proj, fn=merit,
        )

    elif M_saddle == "len":
        # LEN: lazy-Hessian variant (Definition E.1 / Theorem E.3 / E.4).
        # Reuses the Jacobian ∇F for m_lazy consecutive CRN calls,
        # reducing Hessian computation by a factor of m_lazy.
        oracle = make_lazy_crn_npe_oracle(problem, npe_gamma)
        return len_restart(
            oracle, problem.operator_F, z0,
            T=inner_T, gamma=npe_gamma,
            m=params.m_lazy,                  # ← Hessian reuse interval
            S=params.S_inner,
            project=proj, fn=merit,
        )
        ...
        raise ValueError(
            f"Unknown M_saddle={M_saddle!r}; expected 'npe' or 'len'."
        )
